...
- adiciona o sistema visual versionado F1–F6 com grid, safe areas, kernel 1:1,
  tipografia medida e paleta canônica;
- gera 13 diagramas técnicos distintos e integra os 13 medalhões recebidos sem
  alterar os originais;
- conecta o novo renderer à fachada existente com fallback `--legacy-v1`;
- adiciona cards 9:16/1:1, overlays medidos, três provas motion FFmpeg e
  empacotamento determinístico com manifesto SHA-256;
- acrescenta contrato Markdown↔IA v4 com YAML imutável, corpo humano, JSON
  validável, schema, validação Pydantic opcional e publicação transacional;
- adiciona comandos `diagnosticar-v4`, `gerar-roteiro-v4`,
  `validar-roteiro-v4` e `aplicar-roteiro-v4`;
- integra módulos editoriais locais para cor, áudio, captions, capítulos,
  presets, metadados, versões, workflow e busca estrutural;
- corrige o renderer de vídeo pronto para usar overlays v2 medidos, validar o
  MP4 temporário e manter fallback FFmpeg quando MoviePy não existe;
- aceita `--version` e preserva `--versao`;
- inclui instalador lado a lado, documentação de rollback e relatórios de QA.

### Estado

- candidato validado com testes sintéticos e smoke integral;
- revisão visual humana permanece obrigatória antes de publicação;
- ferramentas opcionais detectadas como `discovery_only` não são anunciadas
  como pipelines implementados.
