...
def style_signature(
    ...
    if style.get("design_system", {}).get("enabled"):
        from fr_v4.style_packs.registry import signature as v2_signature
        # O plano já possui hash próprio no registro de prévias. A assinatura
        # visual deve ser estável quando chamada isoladamente pelo Studio.
        digest.update(v2_signature(APP_ROOT, {}, style).encode("ascii"))
...
def save_card_image(canvas: Any, path: Path) -> None:
    """Salva cards atomicamente e verifica o PNG antes de publicá-lo."""
    from PIL import Image
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        if path.suffix.lower() == ".png":
            ...
            output.save(temporary, format="JPEG", quality=96)
        temporary.replace(path)
    finally:
        temporary.unlink(missing_ok=True)
        output.close()
...
def parser() -> argparse.ArgumentParser:
    ...
    root.add_argument("--versao", "--version", action="version", version=f"FR AutoEdite {APP_VERSION}")
    ...
    visual_mode = cards.add_mutually_exclusive_group()
    visual_mode.add_argument("--sistema-v2", action="store_true", help="ativar FR Quiet Engineering Atelier")
    visual_mode.add_argument("--legacy-v1", action="store_true", help="usar compositor visual anterior")
    diagnose_v4 = commands.add_parser("diagnosticar-v4", help="diagnosticar capacidades locais sem instalar dependências")
    diagnose_v4.add_argument("--check", action="store_true", help="executar autotestes sintéticos disponíveis")
    generate_v4 = commands.add_parser("gerar-roteiro-v4", help="gerar o contrato Markdown↔IA v4")
    generate_v4.add_argument("--projeto", required=True)
    validate_v4 = commands.add_parser("validar-roteiro-v4", help="validar contrato v4 sem aplicar")
    validate_v4.add_argument("--projeto", required=True)
    validate_v4.add_argument("--arquivo", required=True)
    apply_v4 = commands.add_parser("aplicar-roteiro-v4", help="validar e publicar contrato v4 de forma transacional")
    apply_v4.add_argument("--projeto", required=True)
    apply_v4.add_argument("--arquivo", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reindexar-assets":
            ...
            return 0
        if args.command == "diagnosticar-v4":
            from fr_v4.adapters.registry import build_registry
            print(json.dumps(build_registry().diagnose(check=bool(args.check)), ensure_ascii=False, indent=2))
            return 0
        if args.command == "gerar-roteiro-v4":
            from fr_v4.contracts.cli import generate_file
            target = generate_file(expand_path(args.projeto), APP_ROOT)
            info(f"Contrato v4 gerado: {target}")
            return 0
        if args.command in {"validar-roteiro-v4", "aplicar-roteiro-v4"}:
            from fr_v4.contracts.cli import validate_file
            result = validate_file(expand_path(args.projeto), expand_path(args.arquivo), APP_ROOT,
                                   apply=args.command == "aplicar-roteiro-v4")
        ...
        if args.command == "cards":
            ...
            if args.sistema_v2 or args.legacy_v1:
                selected_style = load_card_style(project_dir)
                selected_style.setdefault("design_system", {}).update({
                    "enabled": bool(args.sistema_v2), "legacy_v1": bool(args.legacy_v1),
                })
                write_json(project_dir / "CARD_STYLE.json", selected_style)
            if args.plano:
